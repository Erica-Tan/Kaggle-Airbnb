...
train.f5 <- mutate(train_data, first_active_peroid = factor (first_active_year > 2013, labels = c("Before 2013", "After 2013")))

# Summarize a dataset by country_destination and agegroup
train.f6 <- group_by(train.f5, country_destination, first_active_peroid) %>%
    summarise(sum = n())

totalBefore <- sum(train.f6[train.f6$first_active_peroid == 'Before 2013',]$sum)
totalAfter <- sum(train.f6[train.f6$first_active_peroid == 'After 2013',]$sum)
